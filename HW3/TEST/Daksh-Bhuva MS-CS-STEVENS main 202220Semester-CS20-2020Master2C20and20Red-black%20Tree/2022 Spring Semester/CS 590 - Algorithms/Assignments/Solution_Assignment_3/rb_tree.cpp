...
{ /*<<<*/ 
/*
 * delete nil element and all the other allocated nodes
 */
  remove_all(T_root);
  delete T_nil;
} /*>>>*/
...
int i = 0;
void rb_tree::convert_rb(rb_tree_node* x, int* array)
{
    if (x != T_nil)
    {
        convert_rb(x->left, array);
        array[i] = x->key;
        i++;

        convert_rb(x->right, array);
    }
}
...
int rb_tree::check_black_height(rb_tree_node* x)
{
    ...
    int BlackHeight_left = check_black_height(x -> left);
    int BlackHeight_right = check_black_height(x -> right);

    if(BlackHeight_left == 0)
        return BlackHeight_left;
    if (BlackHeight_right == 0)
        return BlackHeight_right;
    if (BlackHeight_left != BlackHeight_right)
        return 0;
    else
        return BlackHeight_left + (x->color == RB_BLACK ? 1:0) ;
}
